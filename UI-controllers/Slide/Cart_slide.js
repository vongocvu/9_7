...
import { Slider } from "../../utils/Slides.js";
import {
  show_cart,
  ProductInCart,
} from "../../UI-controllers/Carts/AddToCart.js";
...
function edit_quantity(prevs, nexts) {
  ...
  prevs.forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      btn.parentElement.children[1].innerText = count;
    });
  });
    ...
    btn.addEventListener("click", () => {
      if (
        ...
        AllProducts.forEach((product, index) => {
          if (product.id === parseInt(btn.getAttribute("product_id"))) {
            ProductInCart.unshift({ ...product, quantity: 1 });
          }
        });
      }
